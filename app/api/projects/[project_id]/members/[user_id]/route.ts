...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  try {
    ...
    await recordAudit({ orgId: gate.project.orgId, actor: gate.user, action: 'project.member.role_changed', targetType: 'project', targetId: project_id, meta: { userId: user_id, role: body.role } });
...
export async function DELETE(_req: NextRequest, { params }: Ctx) {
  try {
    ...
    await recordAudit({ orgId: gate.project.orgId, actor: gate.user, action: 'project.member.removed', targetType: 'project', targetId: project_id, meta: { userId: user_id } });
    return createSuccessResponse(await getProjectAccess(project_id));
  } catch (error) {
