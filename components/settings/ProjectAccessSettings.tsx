...
export default function ProjectAccessSettings({ projectId }: Props) {
  ...
  const avatar = (m: Member) =>
    m.image ? (
      ...
      <div className="w-7 h-7 rounded-full bg-gray-200 dark:bg-gray-800 flex items-center justify-center text-xs font-medium text-gray-600 dark:text-gray-300">
  ...
  return (
    <div className="p-6 space-y-5">
      <div>
        <h3 className="text-lg font-medium text-gray-900 dark:text-gray-50 mb-1">Access</h3>
        <p className="text-sm text-gray-600 dark:text-gray-300">Control who in your organization can see and open this project.</p>
      </div>
      ...
      <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700">
        <div className="pr-4">
          <p className="font-medium text-gray-900 dark:text-gray-50">Restrict to specific users</p>
        ...
        <>
          ...
          <div ref={boxRef} className="relative">
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-200"
            />
            {open && (
              <div className="absolute z-10 mt-1 w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg max-h-60 overflow-y-auto">
                {searching ? (
                  ...
                  results.map((u) => (
                    <button
                      ...
                      disabled={busy}
                      className="flex w-full items-center gap-3 px-3 py-2 text-left hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50"
                    >
                      {avatar(u)}
                ...
                {access!.members.map((m) => (
                  <li key={m.id} className="flex items-center gap-3 px-3 py-2">
                    ...
                    <button
                      ...
                      disabled={busy}
                      className="px-2.5 py-1 text-xs font-medium border border-gray-200 dark:border-gray-700 rounded-full bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-40"
                    >
                      Remove
                    </button>
                  </li>
